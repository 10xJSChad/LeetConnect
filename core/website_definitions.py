...
class CodewarsDefinition(WebsiteDefinition):
    ...
    def parse_result(self, result: str, markdown=False) -> str:
        ...
        if markdown:
            ...
            return [ResultText(
                    text="\n".join([a for a in ("".join(
                    [a for b in result for a in b if "cm-" not in a and a])
                    ).split("\n") if a != ""]).replace("{{NEWL}}", "\n"))]
    ...
    def script_get_result_output(self) -> str:
        return (run_script_until_valid, 
                "return document.getElementById('runner_frame');",
                switch_to_frame, 
                "return document.querySelector('.run-output').outerHTML;")
    ...
    def script_get_result_ready_element(self) -> str | tuple[function | str]:
        return (run_script_until_valid, 
                "return document.getElementById('runner_frame');",
                switch_to_frame, 
                "return document.querySelector('.run-results').outerHTML;")
